...
def init_process(dev_usb):
    ...
    if CONFIG['ONLINE_MODE'] == True:
        ...
        p_read_data = Process(target = get_data, args = (get_data_control,
                                                         get_data_warnings,
                                                         dev_usb,data_queue))

    else:
        from capture import get_data_from_file
        p_read_data = Process(target = get_data_from_file,
                              args = (get_data_control, get_data_warnings,
                                      data_queue))
    ...
    p_procesing = Process(target = data_processing, 
                          args = (data_queue, ui_config, graph_data_queue, 
                                 procesing_control, procesing_warnings))
    
    processing_process = Processing_process_handle(p_procesing, 
                                                   ui_procesing_control,
                                                   procesing_warnings, 
                                                   ui_config,
                                                   graph_data_queue)
...
class Processing_process_handle():
    def __init__(self, process, proccesing_control, proccesing_warnings, 
                 ui_config_queue, graph_data_queue):
